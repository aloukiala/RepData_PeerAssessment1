...
Data that is used in this study represents activity monitoring device data, that is colleted in 5 minute intervals. The data contains date, interval and amount of steps recorded. Data is read from a csv file.
...
The peak interval is `r strftime(filter(timeSteps, stepCount == max(stepCount))$interval, "%H:%M")`. This would indicate that people tend to move before they arrive to work (asumed that work day begins 9:00).
...
Using the average of the interval to fill in the missing values. To do this, we simple take the interval, to which the missing value belongs to and assing it the average value of that interval. 
...
People tend to move more evenly during the day on weekends as for the week, the steps are taken before work. The main peak that is clearly seen during the week around 8:30 is alot more melow during the weekend.
